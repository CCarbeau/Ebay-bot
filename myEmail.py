...
def email(emailAddresses,targets):
    if(targets.size != 0):
        ...
        header = "Item:                 Price:     Discount:    Link:\n"
        email.attach(MIMEText(header,"plain"))
        for row, value in targets.iterrows():
            ...
            line = str(item)+ " " + str(price) + " " + str(discount) + " "
            email.attach(MIMEText(line,"plain"))
...
"""
        ...
        for address in emailAddresses:
            with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context) as smtp:
                ...
                smtp.send_message(myEmail, address, email.as_string())
